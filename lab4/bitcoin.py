...
def padIntTo32Bits(data):
    ...
    splitstring= binarystring.split('b')
    tempstr = ''
    for _ in range(BITS - len(splitstring[1])):
        tempstr = tempstr + str(0)
    finalbinarystring = '0b' + tempstr + splitstring[1]
...
def Ma(A, B, C):
    ...
    for i in range(2, 34):
        if A[i] == B[i] or A[i] == C[i]:
            majoritystring = majoritystring + A[i]
        else:
            majoritystring = majoritystring + C[i]
...
def rightShift(data, amount):
    ...
    x = data[-amount:]
    y = data[2:-amount]
    rightshiftedbit = '0b' + x + y

    return rightshiftedbit
...
def Sum0(A):
    ...
    """
    rotatedstring1 = rightShift(A, 2)
    rotatedstring2 = rightShift(A, 13)
    rotatedstring3 = rightShift(A, 22)

    binarystringsum0 = '0b'

    for i in range(2, 34):
        if rotatedstring1[i] == '1' and rotatedstring2[i] == rotatedstring3[i]:
            binarystringsum0 = binarystringsum0 + str('1')
        elif rotatedstring2[i] == '1' and rotatedstring1[i] == rotatedstring3[i]:
            binarystringsum0 = binarystringsum0 + str('1')
        elif rotatedstring3[i] == '1' and rotatedstring2[i] == rotatedstring1[i]:
            binarystringsum0 = binarystringsum0 + str('1')
        else:
            binarystringsum0 = binarystringsum0 + str('0')

    return binarystringsum0
...
def Ch(E, F, G):
    ...
    for i in range(2, 34):
        if E[i] == '1':
            shuffledstring = shuffledstring + F[i]
        else:
            shuffledstring = shuffledstring + G[i]
...
def Sum1(E):
    ...
    :return: A 32 bit binary string with the result
    """
    rotatedstring1 = rightShift(E, 6)
    rotatedstring2 = rightShift(E, 11)
    rotatedstring3 = rightShift(E, 25)

    binarystringsum1 = '0b'

    for i in range(2, 34):
        if rotatedstring1[i] == '1' and rotatedstring2[i] == rotatedstring3[i]:
            binarystringsum1 = binarystringsum1 + str('1')
        elif rotatedstring2[i] == '1' and rotatedstring1[i] == rotatedstring3[i]:
            binarystringsum1 = binarystringsum1 + str('1')
        elif rotatedstring3[i] == '1' and rotatedstring2[i] == rotatedstring1[i]:
            binarystringsum1 = binarystringsum1 + str('1')
        else:
            binarystringsum1 = binarystringsum1 + str('0')

    return binarystringsum1
